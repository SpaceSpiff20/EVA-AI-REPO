...
async def TextToAudioFile(text) ->  None:
    file_path = r"Data\speech.mp3"   # DEefine the path where the speech will be saved.

    if os.path.exists(file_path): # Check if the file already exists.
        os.remove(file_path)      # If it exists, remove it to avoide overwritting errors.
...
def TTS (Text, func=lambda r=None: True):
    while True:
        try:
            ...
            # Load the generate speech file into pygame mixer.
            pygame.mixer.music.load(r"Data\speech.mp3")
            pygame.mixer.music.play()  # Play the audio.

            # Loop until the audio is done playing or the function stops.
            while pygame.mixer.music.get_busy():
                if func() == False:  # Check is the external function returns false.
                    break
...
if __name__ == "__main__":
    while True:
        # Prompt user for imput and pass it to Text-To-Specch function.
